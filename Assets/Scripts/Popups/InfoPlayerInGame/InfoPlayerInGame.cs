...
public class InfoPlayerInGame : BaseView
{
    ...
    [SerializeField] private GameObject m_buttonAddFriend;
    Player player;
    async public void setInfo(Player _player, bool showActionButtons = true)
    {
        ...
        if (Globals.COMMON_DATA.JsonDataFriend != null)
        {
            JArray listFriens = Globals.COMMON_DATA.JsonDataFriend["listFriend"] as JArray;
            listFriens.ToList().ForEach(item =>
            {
                if ((int)item["userid"] == player.id)
                {
                    m_buttonAddFriend.SetActive(false);
                }
            });
        }

...
public class CreateTableView : BaseView
{
    ...
    public void onClickCreate()
    {
        if (edbPassword.text == "")
        {
            SocketSend.sendCreateTable(currentRoomValue, Globals.Config.typeTable);
        }
        else
        {
            //SocketSend.sendCreateTable(currentRoomValue);
            SocketSend.sendCreateTableWithPass(currentRoomValue, "", edbPassword.text, Globals.Config.typeTable);
        }
        onClickClose();
    }

...
public class TableView : BaseView
{
    ...
    public int typeTable = 0;
    private JArray cachedRoomVipListSolo = null;
    private JArray cachedRoomVipListNormal = null;
    ...
    protected override void Awake()
    {
        ...
        Globals.Config.typeTable = typeTable = 0;
        reloadLtv();
    }
    ...
    protected override void OnEnable()
    {
        ...
        if (Config.isChangeTable)
        {
            ...
            if (Config.listGamePlaynow.Contains(Config.curGameId))
            {
                ...
                SocketSend.sendPlayNow(Config.curGameId, Config.typeTable);
            }
            else
            {
                Debug.Log("==-=-=- doi ban  sendChangeTable");
                SocketSend.sendChangeTable(Config.tableMark, Config.tableId, Config.typeTable);
            }
        }
        ...
        if (Config.curGameId == (int)GAMEID.PUSOY || Config.curGameId == (int)GAMEID.BORKDENG)
        {
            nodeJackpot.Stop();
    ...
    public void ChangeTypePlay()
    {
        ...
        {
            Globals.Config.typeTable = typeTable = 1;
            ...
            // Lấy lại từ cache nếu có
            if (cachedRoomVipListSolo != null)
            {
                room_vip_list = new JArray(cachedRoomVipListSolo);
                reloadListSelect();
            }
            else
            {
            ...
            // Ẩn tất cả các phần tử trong scrTable khi sang solo
            for (int i = 0; i < scrTable.content.childCount; i++)
            {
        ...
        else
        {
            ...
            if (cachedRoomVipListNormal != null)
            {
                room_vip_list = new JArray(cachedRoomVipListNormal);
                reloadListSelect();
            }
            else
            {
    ...
    public void handleListTable(JObject jData)
    {
        ...
        if (Globals.Config.typeTable == 1)
            cachedRoomVipListSolo = new JArray(room_vip_list);
        else
            cachedRoomVipListNormal = new JArray(room_vip_list);
        reloadListSelect();
    }
    ...
    void reloadListSelect()
    {
     ...
     .ToList();
        if (validRooms.Count == 0)
        {
            ...
            for (var i = 0; i < scrTable.content.childCount; i++)
                scrTable.content.GetChild(i).gameObject.SetActive(false);
            return;
        }
    ...
    {
        typeTable = Globals.Config.typeTable;
        Debug.Log("xem type" + typeTable);
        if (typeTable == 0)
        {
            m_ListText[1].color = Color.white;
            m_ListText[0].color = new Color32(0xA5, 0xB1, 0xFD, 0xFF);
    ...
    {
        Debug.Log(typeTable + "xem chế độ solo");
        SoundManager.instance.soundClick();
        SocketSend.sendPlayNow(Config.curGameId, typeTable);
    ...
    public void onClickFindTablle()
    {
        ...
        if (tableId > 0)
        {
            SocketSend.sendCheckPass(tableId, typeTable);
    ...
    private string currentInviteMsg = null;
    public void showInvite(JObject jData)
    {
        ...
        // Nếu popup đang mở và nội dung giống nhau thì giữ nguyên
        if (dialogInvite != null && currentInviteMsg == msg)
        {
            return;
        }
        // Nếu popup đang mở nhưng nội dung khác thì đóng popup cũ
        if (dialogInvite != null)
        {
            dialogInvite.hide();
            dialogInvite = null;
        }
        currentInviteMsg = msg;

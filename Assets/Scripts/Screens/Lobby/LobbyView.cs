...
public class LobbyView : BaseView
{
    ...
    protected override void OnEnable()
    {
        ...
        if (Config.isChangeTable)
        {
            ...
            if (Config.listGamePlaynow.Contains(Config.curGameId)) SocketSend.sendPlayNow(Config.curGameId, Config.typeTable);
            else SocketSend.sendChangeTable(Config.tableMark, Config.tableId, Config.typeTable);
        }
    ...
    {
        Debug.Log("có chạy vào disable ko");
        isChatRunning = false;
        chatQueue.Clear();
    ...
    public void updateBannerNews()
    {
        ...
        Debug.Log("có chạy vào cái updateBanner đấy");
    ...
    public void onClickDailyBonus()
    {
        ...
        UIManager.instance.showToast("សូមរង់ចាំ! វឹងមកដល់នៅពេលឆាប់ៗនេះ!");
    }
    ...
    {
        SoundManager.instance.soundClick();
        Debug.Log("xem support" + Config.is_bl_fb);
        if (Config.is_bl_fb)
        {

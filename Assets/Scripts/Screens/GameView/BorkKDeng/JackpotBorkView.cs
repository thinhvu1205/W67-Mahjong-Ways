...
public class JackpotBorkView : BaseView
{
    public static JackpotBorkView instance = null;
    [SerializeField]
    ...
    public void SetInfo(JObject data)
    {
        ...
        List<JObject> lswin = dataJP["lswin"]
            .ToObject<List<JObject>>()
            .OrderByDescending(obj => (long)obj["timeWin"])
            .ToList();

        for (var i = 0; i < lswin.Count; i++)
        {
            JObject dataPl = lswin[i];
            ...
            item.transform.Find("textPlayer").GetComponent<TextMeshProUGUI>().text = ObjectParse.getString(dataPl, "username");
            item.transform.Find("textReward").GetComponent<TextMeshProUGUI>().text = ObjectParse.getString(dataPl, "markWin");

...
public class BorkKDengView : GameView
{
    ...
    [SerializeField] private GameObject m_ContainerChip;
    ...
    public override void HandlerTip(JObject data)
    {
        ...
        for (int j = 0; j < 2; j++) // sinh ra 4 chip                     //  lay tien nguoi` thua
        {
            ...
            temp.transform.SetParent(m_ContainerChip.transform);
            temp.transform.localPosition = playerTip.playerView.transform.localPosition;
            temp.transform.localScale = new Vector2(0.55f, 0.55f);
            DOTween.Sequence()
    ...
    public override void handleJTable(string strData)
    {
        ...
        {
            if (currentDealer == 0 && thisPlayer.ag >= 100 * agTable)
            {
    ...
    public override void handleSTable(string strData)
    {
        ...
        {
            btn_FindBanker.gameObject.SetActive(false);
            Player pl = getPlayerWithID(currentDealer);
            pl.setDealer(true, pl.playerView.transform.localPosition.x < 0);
        }
        else
        {
            if (thisPlayer.ag >= agTable * 100 && players.Count > 1 && findDealer)
            {
                btn_FindBanker.gameObject.SetActive(true);
            }
            else
    ...
    public override void handleCTable(string strData)
    {
        ...
        Debug.Log("HandleCTable" + strData);
        JObject data = JObject.Parse(strData);
        acceptBanker = (int)data["F"];
        bool findDealer = (bool)data["findDealer"];
        if (thisPlayer.ag >= agTable * 100 && players.Count > 1 && findDealer)
        {
            btn_FindBanker.gameObject.SetActive(true);
        }
    ...
    public override void handleVTable(string strData)
    {
        ...
        if (currentDealer != 0)
        {
            dealerInGame.icDealer.SetActive(false);
        }
        else
        {
            dealerInGame.icDealer.SetActive(true);
            foreach (Player player in players)
            {
                    ...
                    if (idPlayer != dealerId)
                    {
                        ...
                        if (player != null)
                        {
                            ...
                            if (agChange > 0)
                            {
                                ...
                                player.ag = agPlayer;
                                player.setAg();
                                playerView.effectFlyMoney(agChange, 50);
                                playerView.setEffectWin("", false);
                                if (player == thisPlayer)
                                {
    ...
    public void handleLeaveDealer(JObject data)
    {
        ...
        {
            if (thisPlayer.ag >= agTable * 100 && players.Count > 1)
            {
                btn_FindBanker.gameObject.SetActive(true);
            }
            else
    ...
    private void dispatchCardForPlayer(Player player, List<int> arrCardID, bool isDispatch = true)
    {
        ...
        int index = 0;
        float cardScale = player == thisPlayer ? 0.55f : 0.4f;
            ...
            seq.AppendCallback(() =>
            {
                ...
                if (index == l - 1) //la cuoi cung
                {
                    ...
                    else
                    {
                        ...
                        cardScale = score != 0 ? 0.55f : cardScale;
    ...
    private void dispatchCardForDealer(bool isBc, List<int> arrId = null)
    {
        ...
        if (isBc)
        {
            ...
            cardObj.transform.localScale = new Vector2(0.45f, 0.45f);
            cardObj.setTextureWithCode(0);
        ...
        else
        {
            ...
            shorCardOnHand(arrCardDealer, arrId, 0.45f);
    ...
    private void shorCardOnHand(List<Card> cards, List<int> arrId, float cardScale = 0.35f, PlayerViewBorkKDeng player = null, bool isShowScore = false)
    {
        ...
        for (int i = 0; i < cards.Count; i++)
        {
            ...
            {
                DOTween.Sequence()
                          .AppendInterval(isBocCard != true ? 0.35f : 0.1f)
                          .AppendCallback(() =>
                          {
                              if (!isBocCard)
                              {
    ...
    public int HamTinhDiem(int diem)
    {
        ...
        Debug.Log("xem là nó bao nhiêu điểm" + diem);
        return diem;
    }
    ...
    private void dealerShowScore()
    {
        ...
        if (arrNum == 2 && dealerScore == 9)
        {
            ...
            animQiuDealer.AnimationState.SetAnimation(0, "bork9_cam", false);
            //rtDealerBgScore.sizeDelta = new Vector2(210, 74);
        }
        else if (arrNum == 2 && dealerScore == 8)
        {
            lbScore.gameObject.SetActive(false);
            dealerBgScore.GetComponent<Image>().enabled = false;
            animQiuDealer.gameObject.SetActive(true);
            animQiuDealer.Initialize(true);
            animQiuDealer.AnimationState.SetAnimation(0, "bork8_cam", false);
        }
        else
        {
    ...
    public string getTextScore(int score, int numCard)
    {
        ...
        else if (numCard == 3)
        {
            ...
            txtScore = score + " " + Globals.Config.getTextConfig("diem");
            if (score == 11) txtScore = Globals.Config.getTextConfig("txt_pok_sanh");
            if (score == 12) txtScore = Globals.Config.getTextConfig("txt_pok_tpsanh");
            if (score == 13) txtScore = Globals.Config.getTextConfig("txt_pok_3daunguoi");
    ...
    private void throwChipToPlayer(Player player)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            chip.transform.SetParent(m_ContainerChip.transform);

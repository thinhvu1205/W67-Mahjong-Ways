...
public class GameView : BaseView
{
    ...
    {
        SoundManager.instance.soundClick();
        Debug.Log(stateGame + "xem là như nào");
        if (stateGame == STATE_GAME.VIEWING) return;
    ...
    {
        //// return;
        Globals.Logging.Log("-------------Trang thaionLeave :   " + stateGame + " " + HandleData.DelayHandleLeave);
    ...
    protected Player getPlayer(string namePlayer, bool isGetWithDisplayName = false) // mot so game n lai tra data name theo display name a son ak;
    {
        ...
        for (int i = 0; i < players.Count; i++)
        {
            ...
            // Globals.Logging.Log("Search Player:-->namePl" + players[i].namePl +
            //  "--->DisplayName:" + players[i].displayName+
            //  "---->lbName=" + playerV.txtName.text);
            if (string.Equals(players[i].namePl, namePlayer)
             || displayName.Equals(namePlayer) ||
              playerV != null &&
               playerV.txtName.text.Equals(namePlayer))
            {

...
public class PlayerView : MonoBehaviour
{
    ...
    {
        if (timeCounDown == null) return;

        if (timeCounDown.gameObject != null)
    ...
    public virtual void setEffectWin(string animName = "", bool isLoop = true)
    {
        ...
        if (animName == "")
        {
            animResult.transform.localScale = new Vector3(0.4f, 0.4f, 0.4f);
    ...
    public void ShowEffectWinLose(int effectWinLose)
    {
        switch (effectWinLose)
        {
            case 0:
                setEffectLose(false);
                break;
    ...
    public void effectFlyMoney(long mo, int fonzSize = 50)
    {
        ...
        if (mo < 0)
        {
            Debug.Log("có chạy vào chỗ format tiền mà nhỉ");
        ...
        if (Globals.Config.curGameId == (int)Globals.GAMEID.CATTE)
        {
            height = 60;
        }

...
public class DragonTigerView : GameView
{
    ...
    private void moveAnimDTNormal()
    {
        ...
        for (int i = 0; i < listAniStart.Count - 1; i++)
        {
            ...
            listAniStart[i].gameObject.transform.DOScale(new Vector2(1f, 1f), 0.5f);
        }
        listAniStart[2].gameObject.SetActive(false);
        listAniStart[0].gameObject.transform.DOLocalMove(new Vector2(-320, 207), 0.5f);
        listAniStart[1].gameObject.transform.DOLocalMove(new Vector2(320, 207), 0.5f);
    }
    ...
    public void handleLc(JObject dataLc)
    {
            ...
            .AppendCallback(() =>
        {
            ...
            cardDraPref.gameObject.transform.localPosition = new Vector2(-59.5f, -1);
            cardTigPref.gameObject.transform.localPosition = new Vector2(172.62f, -1);
    ...
    public void setAniVs()
    {
        ...
        imageVS.transform.DOLocalMove(new Vector2(0, 0), 0.5f);
    ...
    {
        imageVS.SetActive(true);
        imageVS.transform.localPosition = new Vector2(0, 0);
        imageVS.transform.DOScale(new Vector2(1.5f, 1.5f), 0.5f);
    }
    ...
    public void setAniVsHide()
    {
        ...
        imageVS.transform.localPosition = new Vector2(0, 0);
        imageVS.transform.DOScale(new Vector2(2f, 2f), 0.5f);
    }
    ...
    private void handleChipWinForPlayers(JObject data)
    {
        ...
        inforUser.ForEach((dataUser) =>
        {
            ...
            int Dragon = getInt(dataUser, "Dragon");
            int Tiger = getInt(dataUser, "Tiger");
            int Tie = getInt(dataUser, "Tie");
            int DragonBig = getInt(dataUser, "DragonBig");
    ...
    public override void handleSTable(string strData)
    {
        ...
        if (betsToken != null && betsToken.Type == JTokenType.Array && ((JArray)betsToken).Count > 0)
        {
            ...
            listValueChipBets = betsArray.ToObject<List<long>>();
    ...
    public void onClickBet(int gateBet)
    {
        ...
        {
            listPot[gateBet - 1].transform.GetChild(3).gameObject.SetActive(false);
        });
    ...
    public void effectWinGate(int index)
    {
        ...
        for (int i = 0; i < listPot.Count; i++)
        {
            if (i == resultWin)
            {
                ...
                GameObject btnPress = objButton.transform.GetChild(0).gameObject;
                if (btnPress != null)
                {
